...
<div id="wrapper">
  <div class="content">
    <div class="row">
      <?php
      echo form_open_multipart($this->uri->uri_string(), array('id' => 'debit-note-form', 'class' => '_transaction_form debit-note-form'));
      if (isset($debit_note)) {
          ...
          <div class="panel-body">
            ...
            if (isset($attachments) && count($attachments) > 0) {
              foreach ($attachments as $value) {
                ...
                $path = get_upload_path_by_type('purchase') . 'debit_note/' . $value['rel_id'] . '/' . $value['file_name'];
                $is_image = is_image($path);
                if ($is_image) {
                  echo '<div class="preview_image">';
                }
            ?>
                <a href="<?php echo site_url('download/file/debit_note/' . $value['id']); ?>" class="display-block mbot5" <?php if ($is_image) { ?> data-lightbox="attachment-purchase-<?php echo $value['rel_id']; ?>" <?php } ?>>
                ...
                <?php
                // echo '</div>';
                echo '<a href="' . admin_url('purchase/delete_debit_note_attachment/' . $value['id']) . '" class="text-danger _delete">' . _l('delete') . '</a>';
                ?>
